#!/system/bin/sh
# crash_guard.sh — brick-prevention watchdog for SMB400 mirakc.
#
# Background:
...
while true; do
    ...
    if [ "$cd_count" -gt "$CD_MAX" ]; then
        echo "[crash_guard] crash_dump32 count=$cd_count > $CD_MAX — killing chain + mirakc" >> "$LOG"
    ...
    # --- 3. hard floor: kill mirakc if still critical after reclaim ---
